...
class ResourcesTestCase(unittest.TestCase):
    def test_get_resource_name(self):
        self.assertEqual(
            get_resource_name(namespace='stuff', path_components=('foo', 'bar')),
            'stuff:foo/bar')

    def test_get_advancement_path(self):
        self.assertEqual(
            get_advancement_path(root='.', namespace='stuff', path_components=('foo', 'bar')),
            '{}.{}'.format(os.sep.join(('.', 'data', 'stuff', 'advancements', 'foo', 'bar')), 'json'))

    def test_get_function_path(self):
        self.assertEqual(
            get_function_path(root='.', namespace='stuff', path_components=('foo', 'bar')),
            '{}.{}'.format(os.sep.join(('.', 'data', 'stuff', 'functions', 'foo', 'bar')), 'mcfunction'))

    def test_get_loot_table_path(self):
        self.assertEqual(
            get_loot_table_path(root='.', namespace='stuff', path_components=('foo', 'bar')),
